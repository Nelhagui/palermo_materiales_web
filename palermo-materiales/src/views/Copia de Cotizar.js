...
const Cotizar = () => {
  ...
  function handleCategories(categoria_id) {
    setSelectedFilter([c])
  ...
  function fetchCategories(categoria_id) 
  {
    axios
    .get(
      `https://test.api.palermomateriales.com.ar/api/categoria/cotizable/${categoria_id}`,
    )
    .then((res) => 
      {
    ...
    <div className="wrapper  ">
      ...
      <div className="cotizar-subcategorias mt-5 container">
        {subcategories?.map((k) => {
          if (k.tipo == 'categoria') {
            return (
              <div className="cotizar-subitems" onClick={() => handleClick(k)} key={Math.random()}>
